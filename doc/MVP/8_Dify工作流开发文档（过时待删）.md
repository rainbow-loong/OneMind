...
3.  **长期记忆 (MemoBase)**: 使用 **代码节点** 与外部数据库（Supabase）交互，实现对用户“整合结晶 (Integration Crystal)”的长期记录与读取，达成“修身”效果。
...
2.  **`integration_crystals` (整合结晶/长期记忆表)**
...
*   **代码 (Python)**:
    ...
    def main(args: dict) -> dict:
        ...
        if crystal_data.data:
            long_term_memory_insight = f"根据你上次的整合结晶《{crystal_data.data[0]['name']}》，你似乎在学习如何与'{crystal_data.data[0]['key_insight']}'共处。"
